...
if "df" not in st.session_state:
    if os.path.exists(CACHE_PATH):
        ...
        cached_bytes, fetch_error = github_sync.fetch_all_data_bytes()
        if cached_bytes:
            try:
                st.session_state["df"] = load_all_data(io.BytesIO(cached_bytes))
                st.session_state.setdefault("df_filename", "(GitHub에 저장된 최신 파일)")
            except Exception as e:
                st.sidebar.warning(f"GitHub에서 불러온 파일을 읽는 중 문제가 있었어요: {e}")
        elif fetch_error:
            st.sidebar.warning(f"GitHub에서 이전 데이터를 못 불러왔어요: {fetch_error}")
...
with st.sidebar.expander("저장 상태 확인", expanded=False):
    if github_sync.is_configured():
        st.write(f"GitHub 연동: 켜짐 ✅ (저장소: {github_sync._get_config()[1]})")
    else:
        st.write("GitHub 연동: 꺼짐 (Secrets 미설정)")
    st.write(f"로컬 캐시 파일: {'있음' if os.path.exists(CACHE_PATH) else '없음'}")
